-- ============================================================================
-- CORPORATE NOTIFICATIONS TABLE MIGRATION (FIXED VERSION)
-- ============================================================================
...
DROP INDEX IF EXISTS public.idx_notifications_matter_id;
...
DROP TABLE IF EXISTS public.corporate_notifications CASCADE;
...
CREATE INDEX idx_notifications_user_id ON public.corporate_notifications(user_id);
CREATE INDEX idx_notifications_matter_id ON public.corporate_notifications(matter_id);
CREATE INDEX idx_notifications_is_read ON public.corporate_notifications(is_read);
CREATE INDEX idx_notifications_created_at ON public.corporate_notifications(created_at DESC);
CREATE INDEX idx_notifications_user_unread ON public.corporate_notifications(user_id, is_read) WHERE is_read = false;
...
COMMENT ON COLUMN public.corporate_notifications.id IS 'Unique notification identifier (UUID)';
...
SELECT
    'corporate_notifications table created successfully!' as status,
    count(*) as notification_count
FROM public.corporate_notifications;
